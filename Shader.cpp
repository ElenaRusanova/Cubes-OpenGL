...
Shader::Shader(const char* vertexPath, const char* fragmentPath) 
{
	...
	try
	{
		std::stringstream vShaderSteam;
		vShaderFile.open(vertexPath);
		vShaderSteam << vShaderFile.rdbuf();
		vShaderFile.close();
		vTempString = vShaderSteam.str();
		vShaderCode = vTempString.c_str();

		std::stringstream fShaderSteam;
		fShaderFile.open(fragmentPath);
		fShaderSteam << fShaderFile.rdbuf();
		fShaderFile.close();
		fTempString = fShaderSteam.str();
...
void Shader::setFloatVec(const std::string& name, float* vec, int vec_size) const
{
	...
	default:
		std::cout << "SHADER FAILURE! NO SUCH UNOFORM VECTOR SIZE! " << std::endl;
	}
}
